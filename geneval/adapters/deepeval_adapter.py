...
from deepeval.models import AmazonBedrockModel, AnthropicModel, AzureOpenAIModel, DeepEvalBaseLLM, DeepSeekModel, GeminiModel, GPTModel, OllamaModel
...
class VLLMModel(DeepEvalBaseLLM):
    ...
    def __init__(
        ...
        self.sync_client = httpx.Client(verify=self.ssl_verify, timeout=timeout)

        self.async_client = httpx.AsyncClient(verify=self.ssl_verify, timeout=timeout)
    ...
    def generate(self, prompt: str) -> str:
        ...
        try:
            ...
            response = self.sync_client.post(endpoint_url, json=payload, headers=headers)
            response.raise_for_status()
            return response.json()["choices"][0]["message"]["content"]

        except httpx.HTTPStatusError as e:
            raise RuntimeError(f"HTTP error calling vLLM API: {e.response.status_code} - {e.response.text}") from e
        except httpx.ConnectError as e:
            raise RuntimeError(f"Connection error connecting to vLLM server: {e}") from e
    ...
    async def a_generate(self, prompt: str) -> str:
        ...
        try:
            ...
            response = await self.async_client.post(endpoint_url, json=payload, headers=headers)
            response.raise_for_status()
            return response.json()["choices"][0]["message"]["content"]
...
class DeepEvalAdapter:
    ...
    def __init__(self, llm_manager: LLMManager):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to initialize DeepEval metrics: {e}") from e
    ...
    def _create_model(self) -> None:
        ...
        if provider == "vllm":
            ...
            self.logger.info(
                f"Created custom vLLM model: {deepeval_config['model']}, "
                f"endpoint: {deepeval_config.get('base_url')}{deepeval_config.get('api_path', '/v1')}"
            )
            return vllm_model
            ...
            raise ValueError(
                f"Provider {provider} not fully supported for DeepEval. "
                f"Only OpenAI, Azure OpenAI, Anthropic, Amazon Bedrock, Gemini, DeepSeek, Ollama and VLLM providers are currently supported."
            )
